...
```{r}
or_tokens <- get_tokens(collection = c("Eng-NA","Eng-UK"), 
                          corpus = NULL, 
...
or_tokens$target_child_age_years <- 
  or_tokens$target_child_age %>% 
  duration("days") %>% 
  as.numeric("years")

or_tokens %>%
  filter(target_child_age_years<2, speaker_role=="Target_Child")
```
